...
export class WebTypingsInstallerClient implements ts.server.ITypingsInstaller {
	 ...
	 */
	private async handleResponse(response: InstallerResponse): Promise<codemavi> {
	...
	enqueueInstallTypingsRequest(p: ts.server.Project, typeAcquisition: ts.TypeAcquisition, unresolvedImports: ts.SortedReadonlyArray<string>): codemavi {
	...
	attach(projectService: ts.server.ProjectService): codemavi {
		this.projectService = projectService;
	}

	onProjectClosed(_projectService: ts.server.Project): codemavi {
		// noop
	}
}
...
class WebTypingsInstallerServer extends ts.server.typingsInstaller.TypingsInstaller {
	...
	private constructor(
		...
		private readonly handleResponse: (response: InstallerResponse) => codemavi,
	...
	static async initialize(
		handleResponse: (response: InstallerResponse) => codemavi,
		fs: ts.server.ServerHost,
		globalTypingsCachePath: string,
	 ...
	 */
	protected override installWorker(requestId: number, packageNames: string[], cwd: string, onRequestCompleted: ts.server.typingsInstaller.RequestCompletedAction): codemavi {
	 ...
	 */
	protected override sendResponse(response: InstallerResponse): codemavi {
