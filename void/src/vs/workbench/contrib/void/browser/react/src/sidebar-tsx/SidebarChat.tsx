...
const titleOfBuiltinToolName = {
	...
	'create_checkpoint': { done: `Created checkpoint`, proposed: `Create checkpoint`, running: loadingTitleWrapper(`Creating checkpoint`) },
	'ask_user': { done: `Asked user`, proposed: `Ask user`, running: loadingTitleWrapper(`Asking user`) },
	'delegate_to_executor': { done: `Delegated to Executor`, proposed: `Delegate to Executor`, running: loadingTitleWrapper(`Delegating to Executor`) },
	'delegate_to_verifier': { done: `Delegated to Verifier`, proposed: `Delegate to Verifier`, running: loadingTitleWrapper(`Delegating to Verifier`) },
...
const toolNameToDesc = (toolName: BuiltinToolName, _toolParams: BuiltinToolCallParams[BuiltinToolName] | undefined, accessor: ReturnType<typeof useAccessor>): {
	...
	const x = {
		...
		'create_checkpoint': () => {
			const toolParams = _toolParams as BuiltinToolCallParams['create_checkpoint']
			return {
				desc1: toolParams.message,
			}
		},
		'ask_user': () => {
			const toolParams = _toolParams as BuiltinToolCallParams['ask_user']
			return {
				desc1: toolParams.question,
			}
		},
		'delegate_to_executor': () => {
			const toolParams = _toolParams as BuiltinToolCallParams['delegate_to_executor']
			return {
				desc1: toolParams.task,
			}
		},
		'delegate_to_verifier': () => {
			const toolParams = _toolParams as BuiltinToolCallParams['delegate_to_verifier']
			return {
				desc1: toolParams.checks,
			}
		}
	}
...
const builtinToolNameToComponent: { [T in BuiltinToolName]: { resultWrapper: ResultWrapper<T>, } } = {
	...
	'ask_user': {
		resultWrapper: ({ toolMessage }) => {
			...
			if (toolMessage.type === 'success') {
				componentParams.children = <ToolChildrenWrapper>
					<div className='text-sm italic text-void-fg-4'>
						Answer: {toolMessage.result.answer}
					</div>
				</ToolChildrenWrapper>
			}
