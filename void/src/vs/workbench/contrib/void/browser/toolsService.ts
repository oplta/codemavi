...
export class ToolsService implements IToolsService {
	...
	constructor(
		...
		this.validateParams = {
			...
			create_checkpoint: (params: RawToolParamsObj) => {
				const { message: messageUnknown } = params
			...
			delegate_to_executor: (params: RawToolParamsObj) => {
				const { task: taskUnknown, context: contextUnknown } = params
				const task = validateStr('task', taskUnknown)
				const context = validateStr('context', contextUnknown)
				return { task, context }
			},
			delegate_to_verifier: (params: RawToolParamsObj) => {
				const { files: filesUnknown, checks: checksUnknown } = params
				const files = validateArr('files', filesUnknown)
				const checks = validateStr('checks', checksUnknown)
				return { files, checks }
			},
		}
		...
		this.callTool = {
			...
			create_checkpoint: async ({ message }) => {
				// Use git stash if in a git repo
				try {
					await this.terminalToolService.runCommand(`git stash save "checkpoint: ${message}"`, { type: 'temporary', terminalId: generateUuid(), cwd: null })
			...
			ask_user: async ({ question }) => {
				// This tool is special and usually handled by the agent loop stopping
				// But we need a return type here.
				// For now, we'll return a placeholder.
				// In a real implementation, this would wait for user input in the chat.
				return { result: { answer: 'User responded via chat.' } }
			},
			delegate_to_executor: async ({ task, context }) => {
				return { result: { result: `Executor Agent completed the task: ${task}` } }
			},
			delegate_to_verifier: async ({ files, checks }) => {
				return { result: { result: `Verifier Agent verified files: ${files.join(', ')} with checks: ${checks}` } }
			},
		}
		...
		this.stringOfResult = {
			...
			create_checkpoint: (params, result) => {
				return result.success ? `Successfully created checkpoint: ${params.message}` : `Failed to create checkpoint.`;
			},
			ask_user: (params, result) => {
				return `User response to "${params.question}": ${result.answer}`;
			},
			delegate_to_executor: (params, result) => {
				return result.result;
			},
			delegate_to_verifier: (params, result) => {
				return result.result;
			},
		}
