...
export const builtinTools: {
	...
	create_checkpoint: {
		...
		description: `Creates a checkpoint of the current state of the codebase. You should call this before making any major changes.`,
		params: { message: { description: `A brief message describing the checkpoint.` } }
	},

	ask_user: {
		name: 'ask_user',
		description: `Asks the user a question and waits for their response. Use this when you need clarification or permission for a sensitive action.`,
		params: { question: { description: `The question to ask the user.` } }
	},

	delegate_to_executor: {
		name: 'delegate_to_executor',
		description: `Delegates a specific task to an Executor Agent who will implement the changes.`,
		params: {
			task: { description: `The specific task for the executor.` },
			context: { description: `Relevant context for the task (file contents, rules, etc).` }
		}
	},

	delegate_to_verifier: {
		name: 'delegate_to_verifier',
		description: `Delegates verification of changes to a Verifier Agent.`,
		params: {
			files: { description: `List of files to verify.` },
			checks: { description: `Specific checks to perform (lint, tests, etc).` }
		}
	}
...
export const orchestrator_systemMessage = ({ workspaceFolders, directoryStr, activeURI, openedURIs, persistentTerminalIDs }: { workspaceFolders: string[], directoryStr: string, activeURI: string | undefined, openedURIs: string[], persistentTerminalIDs: string[] }) => {
	return `\
...
<system_info>
- ${os}
- Workspace Folders: ${workspaceFolders.join('\n') || 'NONE'}
- Active File: ${activeURI || 'NONE'}
- Open Files: ${openedURIs.join('\n') || 'NONE'}
- Persistent Terminals: ${persistentTerminalIDs.join(', ') || 'NONE'}
</system_info>
...
## Çalışma Akışın
1. **Analiz ve Planlama**: İsteği analiz et, context topla (search_codebase, read_file), plan oluştur.
2. **Checkpoint**: Her değişiklik öncesi \`create_checkpoint\` çağır.
3. **Alt-Görev Dağıtımı**: Görevleri mantıklı parçalara böl ve araçları kullanarak uygula.
4. **Sonuç Doğrulama**: Değişiklik sonrası \`read_lint_errors\` ile kontrol et.
5. **Hata Yönetimi**: Hata varsa düzelt (max 3 deneme).
6. **Sonuç Sunumu**: Özet sun.

## Kritik Kurallar
- ❌ Dosya içeriğini tahmin etme - MUTLAKA \`read_file\` kullan.
- ✅ Checkpoint oluştur (her değişiklik öncesi).
- ✅ 3+ dosya değişikliğinde \`ask_user\` ile onay al.
- ✅ Hataları \`read_lint_errors\` ile doğrula.

## Araç Kullanımı
Tool calling details:
- To call a tool, write its name and parameters in XML format.
- Format: <tool_name><param1>value</param1></tool_name>
- You must STOP and WAIT for the result after a tool call.
- Only ONE tool call at a time.

Available tools:
${toolCallDefinitionsXMLString(Object.values(builtinTools))}
`.trim().replace('\t', '  ')
}
...
export const chat_systemMessage = ({ workspaceFolders, openedURIs, activeURI, persistentTerminalIDs, directoryStr, chatMode: mode, mcpTools, includeXMLToolDefinitions }: { workspaceFolders: string[], directoryStr: string, openedURIs: string[], activeURI: string | undefined, persistentTerminalIDs: string[], chatMode: ChatMode, mcpTools: InternalToolInfo[] | undefined, includeXMLToolDefinitions: boolean }) => {
	if (mode === 'agent') {
		return orchestrator_systemMessage({ workspaceFolders, directoryStr, activeURI, openedURIs, persistentTerminalIDs })
	}
