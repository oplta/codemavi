...
export class NullFileSystemProvider implements IFileSystemProvider {
	...
	private readonly _onDidChangeCapabilities = new Emitter<codemavi>();
	readonly onDidChangeCapabilities: Event<codemavi> = this._onDidChangeCapabilities.event;
	...
	emitFileChangeEvents(changes: IFileChange[]): codemavi {
		this._onDidChangeFile.fire(changes);
	}

	setCapabilities(capabilities: FileSystemProviderCapabilities): codemavi {
	...
	async mkdir(resource: URI): Promise<codemavi> { return undefined; }
	async readdir(resource: URI): Promise<[string, FileType][]> { return undefined!; }
	async delete(resource: URI, opts: IFileDeleteOptions): Promise<codemavi> { return undefined; }
	async rename(from: URI, to: URI, opts: IFileOverwriteOptions): Promise<codemavi> { return undefined; }
	async copy(from: URI, to: URI, opts: IFileOverwriteOptions): Promise<codemavi> { return undefined; }
	...
	async writeFile(resource: URI, content: Uint8Array, opts: IFileWriteOptions): Promise<codemavi> { return undefined; }
	async open(resource: URI, opts: IFileOpenOptions): Promise<number> { return undefined!; }
	async close(fd: number): Promise<codemavi> { return undefined; }
