...
export class FilterWidget extends Widget {
	...
	private readonly delayedFilterUpdate: Delayer<codemavi>;
	...
	constructor(
		...
		super();
		this.delayedFilterUpdate = new Delayer<codemavi>(300);
	...
	updateBadge(message: string | undefined): codemavi {
		this.filterBadge.classList.toggle('hidden', !message);
		this.filterBadge.textContent = message || '';
		this.adjustInputBox();
	}

	setFilterText(filterText: string): codemavi {
	...
	checkMoreFilters(checked: boolean): codemavi {
	...
	private adjustInputBox(): codemavi {
