...
export class MetricsService implements IMetricsService {
	...
	constructor(
		...
		this.metricsService = ProxyChannel.toService<IMetricsService>(mainProcessService.getChannel('mavi-channel-metrics'));
	}
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: 'maviDebugInfo',
			f1: true,
			title: localize2('maviMetricsDebug', 'Code Mavi IDE: Log Debug Info'),
		});
	}
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		notifService.info(`Code Mavi IDE Debug info:\n${JSON.stringify(debugProperties, null, 2)}`)
	}
})
