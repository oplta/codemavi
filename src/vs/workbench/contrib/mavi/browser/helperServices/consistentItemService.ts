...
type AddItemInputs = { uri: URI; fn: (editor: ICodeEditor) => (() => codemavi); }
...
export interface IConsistentItemService {
	...
	removeConsistentItemFromURI(consistentItemId: string): codemavi;
}
...
export class ConsistentItemService extends Disposable implements IConsistentItemService {
	...
	private readonly disposeFnOfItemId: Record<string, () => codemavi> = {}
...
export interface IConsistentEditorItemService {
	readonly _serviceBrand: undefined;
	addToEditor(editor: ICodeEditor, fn: () => () => codemavi): string;
	removeFromEditor(itemId: string): codemavi;
}
...
export class ConsistentEditorItemService extends Disposable {
	...
	private readonly itemInfoById: Record<
		string,
		{
			editorId: string;
			uriFsPath: string;
			fn: (editor: ICodeEditor) => () => codemavi;
			disposeFn?: () => codemavi;
		}
	> = {};
