...
export class ListSettingListModel<TDataItem extends object> {
	...
	setEditKey(key: EditKey): codemavi {
		this._editKey = key;
	}

	setValue(listData: TDataItem[]): codemavi {
		this._dataItems = listData;
	}

	select(idx: number | null): codemavi {
...
export abstract class AbstractListSettingWidget<TDataItem extends object> extends Disposable {
	...
	protected abstract addTooltipsToRow(rowElement: RowElementGroup, item: TDataItem): codemavi;
	...
	protected editSetting(idx: number): codemavi {
		this.model.setEditKey(idx);
		this.renderList();
	}

	public cancelEdit(): codemavi {
	...
	private onListClick(e: PointerEvent): codemavi {
	...
	private onListDoubleClick(e: MouseEvent): codemavi {
		const targetIdx = this.getClickedItemIndex(e);
		if (targetIdx < 0) {
			return;
		}
	...
	private selectRow(idx: number): codemavi {
	...
	private selectNextRow(): codemavi {
		this.model.selectNext();
		this.selectRow(this.model.getSelected()!);
	}

	private selectPreviousRow(): codemavi {
...
export class ExcludeSettingWidget extends ListSettingWidget<IIncludeExcludeDataItem> {
	...
	protected override addTooltipsToRow(rowElementGroup: RowElementGroup, item: IIncludeExcludeDataItem): codemavi {
...
interface IObjectRenderEditWidgetOptions {
	...
	update(keyOrValue: ObjectKey | ObjectValue): codemavi;
}
...
export class ObjectSettingDropdownWidget extends AbstractListSettingWidget<IObjectDataItem> {
	...
	override setValue(listData: IObjectDataItem[], options?: IObjectSetValueOptions): codemavi {
	...
	protected addTooltipsToRow(rowElementGroup: RowElementGroup, item: IObjectDataItem): codemavi {
...
export class ObjectSettingCheckboxWidget extends AbstractListSettingWidget<IBoolObjectDataItem> {
	...
	override setValue(listData: IBoolObjectDataItem[], options?: IBoolObjectSetValueOptions): codemavi {
		if (isDefined(options) && options.settingKey !== this.currentSettingKey) {
			this.model.setEditKey('none');
			this.model.select(null);
			this.currentSettingKey = options.settingKey;
	...
	private renderEditWidget(
		...
		onValueChange: (newValue: boolean) => codemavi
	) {
	...
	protected addTooltipsToRow(rowElementGroup: RowElementGroup, item: IBoolObjectDataItem): codemavi {
