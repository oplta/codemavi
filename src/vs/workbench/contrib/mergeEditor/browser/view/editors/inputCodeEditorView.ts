...
export class ModifiedBaseRangeGutterItemModel implements IGutterItemInfo {
	...
	public setState(value: boolean, tx: ITransaction): codemavi {
	...
	public toggleBothSides(): codemavi {
		transaction(tx => {
...
export class MergeConflictGutterItemView extends Disposable implements IGutterItemView<ModifiedBaseRangeGutterItemModel> {
	...
	layout(top: number, height: number, viewTop: number, viewHeight: number): codemavi {
	...
	update(baseRange: ModifiedBaseRangeGutterItemModel): codemavi {
		transaction(tx => {
